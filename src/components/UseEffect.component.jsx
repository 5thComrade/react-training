...
// Whenever the state or prop value of the component changes, the component gets re-rendered
...
const UseEffect = () => {
  ...
  useEffect(() => {
    // fetch some data from an api but only when the component is created put the logic here
    console.log(
      "This will only run when the component is created the first time!"
    );
  }, []);

  useEffect(() => {
    // you want to fetch data everytime the useID changes but the logic here

    if (counter > 10) {
      console.log("This will run whenever counter changes!");
    }
  }, [counter]);

  useEffect(() => {
    console.log(
      "Whenever the component gets re-rendered this effect will run!"
    );
  });

  return (
    <section className="mt-8">
      <h1 className="text-2xl font-light">useEffect Hook</h1>
      <p className="text-sm">This component uses useEffect hook</p>
